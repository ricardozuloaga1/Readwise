...
export default function QuizGenerator({ articleText, highlightedText, onQuizComplete }: QuizGeneratorProps) {
  ...
  const handleAnswerSelect = useCallback((questionId: string, answer: string) => {
    console.log('Handling answer:', { questionId, answer }); // Debug log
    setQuestions(prev => {
      const newQuestions = prev.map(q => {
        if (q.id === questionId) {
          console.log('Updating question:', { id: q.id, type: q.type, answer }); // Debug log
          return {
            ...q,
            userAnswer: answer
          };
      ...
      console.log('New questions state:', newQuestions); // Debug log
      return newQuestions;
    });
  }, []);
  ...
  const checkAnswers = async () => {
    ...
    if (user) {
      try {
        ...
        console.log('Quiz result saved, triggering refresh');
        ...
        if (dashboard) {
          ...
          console.log('Dispatched quiz-completed event');
        } else {
          console.error('Dashboard element not found');
        }

        // Call the callback as backup
        onQuizComplete?.();
      } catch (error) {
  ...
  const handleKeyPress = (e: React.KeyboardEvent, questionId: string) => {
    if (e.key === 'Enter' && questions.every(q => q.userAnswer)) {
      checkAnswers();
    }
  };
                  ...
                  <div className="relative">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
                          if (questions.every(q => q.userAnswer)) {
                            checkAnswers();
                          }
                        }
                      }}
                    ...
                    {!showResults && (
                      <div className="mt-1 text-sm text-gray-500">
                        Type your answer and press Enter or click Check Answers when done
                      </div>
                    )}
                  </div>
                  {showResults && (
